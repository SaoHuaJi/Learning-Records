...
> ReAct 可看作是 LLM Agent 发展的一个关键分水岭：它不再只让模型在文本里推理（CoT），也不只让模型照着环境反应式地输出动作（Act-only），而是把二者交错到同一条轨迹里，让模型边想边做、边做边补充信息。
...
下面这张图给出了 Standard / CoT / Act-only / ReAct 的对比，并在 HotpotQA 与 ALFWorld 里展示了 ReAct 轨迹长什么样。
![Prompt](Resources/ReAct%20-%20Prompting%20Comparison.png)

```mermaid
flowchart TD
  O[Observation / Context] --> T["
    Thought: 
    plan / reason / track
  "]
  T --> A["
...
- **与 CoT-SC 的组合策略**
论文发现：ReAct 更稳定，但有时 CoT-SC 能给出更好的最终综合答案。因此作者提出两种启发式混合：
